...
class CONLL2003(data.Dataset):#
    def __init__(self,root):#./data
        ...
        self.testa_length  = len(self.testa)
        self.testb_length  = len(self.testb)
        ...
        with open(path,'r') as fd:
            ...
            for line in fd:
                ...
                if re.match(r"-DOCSTART-.+", line) or (len(line.strip()) == 0):
                    ...
                    curword.append( self.w2i[self.canonicalize_word(tokens[0], wordset=self.w2i)] )
                    curlabel.append( self.t2i[ tokens[1]] )

        return docs
    def gen_tensor(self,sentence,labels):
        twords  = torch.tensor(sentence, dtype=torch.long)
    ...
    def __getitem__(self, index):
        ...
        twords  = torch.tensor(words, dtype=torch.long)
        tlabels = torch.tensor(labels, dtype=torch.long)
        return twords,tlabels
